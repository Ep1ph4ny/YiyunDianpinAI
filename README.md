...
### 鉴于太多对项目前端代码有点疑问，我当时偷懒，随便把前端代码放置了一下，在这里对很多人造成了困扰，这里说声抱歉，下面补充一下前端说明。有技术底蕴的小伙伴肯定发现了对于这个系统，无论是在需求设计还是实现方式上都可以改进。题主最近在研究agent系统（纯兴趣），到时会对整个项目代码重构，给大家一个更加具有教学意义，业界流行的架构项目。这里挖个坑，等我有时间，把项目重构成一下，我将亲自给软件架构重新设计一番（逻辑合理，架构分明），挖个坑。


## 前端说明

本项目的前端不是单独的前端工程，而是 Nginx 托管的静态资源。

前端目录：
`nginx-1.18.0 dianping/html/hmdp/`
...
启动方式：
1. 启动后端 Spring Boot，端口 `8081`
2. 启动 Nginx
3. 访问 `http://localhost:8080`

说明：
- Nginx 静态资源根目录配置在 `nginx-1.18.0 dianping/conf/nginx.conf`
- `/api` 请求会被代理到后端 `127.0.0.1:8081`
## 本地运行前注意

本项目有少量本地环境相关配置，需要按自己的机器修改：

1. 修改 `src/main/resources/application.yaml`
   - MySQL 地址、端口、用户名、密码
   - Redis 地址、端口、密码
   - RabbitMQ 地址、端口、用户名、密码

2. 修改 `src/main/java/com/hmdp/config/RedissonConfig.java`
   - 该文件中 Redis 地址为单独硬编码，需与 `application.yaml` 保持一致

3. 修改 `src/main/java/com/hmdp/utils/SystemConstants.java`
   - `IMAGE_UPLOAD_DIR` 为图片上传目录，当前为作者本机绝对路径
   - 运行前请改为自己本地 `nginx/html/hmdp/imgs` 的实际路径

4. 如修改后端端口，需同步修改
   - `nginx-1.18.0 dianping/conf/nginx.conf`
   - `/api` 代理地址应与 Spring Boot 端口一致
