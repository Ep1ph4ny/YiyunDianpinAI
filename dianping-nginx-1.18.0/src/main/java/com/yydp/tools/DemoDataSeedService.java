...
import com.yydp.service.IUserService;
import com.yydp.utils.CacheClient;
...
public class DemoDataSeedService {
    ...
    @Value("${yydp.seed.shop-per-type:18}")
    private int shopsPerType;

    @Value("${yydp.seed.blogs-per-shop:12}")
    private int blogsPerShop;

    @Value("${yydp.seed.min-users:220}")
    private int minUsers;

    @Value("${yydp.seed.center-x:120.149993}")
    private double centerX;

    @Value("${yydp.seed.center-y:30.334229}")
    private double centerY;

    @Value("${yydp.seed.radius-meters:4600}")
